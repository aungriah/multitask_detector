"""
This script is identical to evaluate.py, with the exception that self-labeled images are used for evaluation
"""
...
def eval_lanes_ft(config, model):
    ...
    config['finetuning_dataloader']['args']['batch_size'] = 1
    config['finetuning_dataloader']['args']['shuffle'] = False

    run_test_tusimple(model, config['finetuning_dataloader']['args']['data_dir'], None, exp_name, config['arch']['griding_num'], False, False, config)
    synchronize()  # wait for all results
    if is_main_process():
        combine_tusimple_test(exp_name)

        res = LaneEval.bench_one_submit(os.path.join(exp_name + '.%d.txt' % get_rank()),
                                        os.path.join(config['finetuning_dataloader']['args']['data_dir'],
                                                     'ft_test.json'))

        # res = LaneEval.bench_one_submit(os.path.join(exp_name + '.txt'),
        #                                 os.path.join(config['finetuning_dataloader']['args']['data_dir'], 'ft_test.json'))
...
# Main test function ------------------------------------------------------------------------------------------------- #
def run_test_tusimple(net,data_root,work_dir,exp_name,griding_num,use_aux,distributed,config, batch_size = 8):
    output_path = os.path.join(exp_name+'.%d.txt'% get_rank())
    fp = open(output_path,'w')
    # loader = get_test_loader(batch_size,data_root,'Tusimple', distributed)
    config['datasets']['lanes'] = 'finetuning'
