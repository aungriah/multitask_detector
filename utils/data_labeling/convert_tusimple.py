"""
Creates binary segmentations masks from lane annotations
"""
...
def generate_segmentation_and_train_list(root, line_txt, names):
    ...
    for i in tqdm.tqdm(range(len(line_txt))):
        ...
        # print(os.path.join(root, label_path))
        # print(root)
        # print(label_path)
        # print(os.path.join(root[:-1], label_path[1:]))
        destination = os.path.join(root, 'segLabel', label_path)
        #print(os.path.join(root, 'segLabel', label_path[1:]))
        cv2.imwrite(destination, label)

        train_gt_fp.write(names[i] + ' ' + 'seglabel/' + label_path + ' ' + ' '.join(list(map(str, bin_label))) + '\n')
    train_gt_fp.close()
